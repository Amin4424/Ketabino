'use client';
// File: app/(author)/author/books/[id]/edit/page.tsx
...
export default function EditBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    if (!book) {
      Promise.resolve().then(() => {
        setError('کتاب پیدا نشد یا به آن دسترسی ندارید');
    ...
    getBookChapters(bookId)
      ...
      .catch((e: unknown) => {
        if (!cancelled) setError((e as Error).message || 'خطا در دریافت فصل‌ها');
      })
      .finally(() => {
  ...
  }, [bookId, books, getBookChapters, isLoading]);

  function toggleGenre(genreId: number) {
    setSelectedGenres(prev => prev.includes(genreId) ? prev.filter(item => item !== genreId) : [...prev, genreId]);
  }

  function updateChapterDraft(index: number, field: keyof ChapterDraft, value: unknown) {
  ...
  function removeChapter(index: number) {
    ...
    setActiveChapter(Math.max(0, Math.min(activeChapter, chapters.length - 2)));
  }

  async function handleSubmit() {
    if (!title.trim()) { setError('عنوان کتاب الزامی است'); return; }
    const invalidChapter = chapters.find(ch => ch.title.trim() && !ch.content.trim());
    if (invalidChapter) { setError('برای فصل‌های دارای عنوان، متن فصل هم الزامی است'); return; }

    setIsSubmitting(true);
    setError('');
    try {
      const nextBookStatus = bookStatus === 'Archived'
        ? 'Archived'
        : chapters.some(ch => ch.status === 'Published') ? 'Published' : 'Draft';
  ...
  if (authLoading || isLoading || isHydrating) {
    return (
  ...
  if (!isAuthenticated || (user?.role !== 'Author' && user?.role !== 'Admin')) return null;
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '12px 20px 32px' }}>
      ...
      <h1 style={{ fontWeight: 900, fontSize: '1.5rem', marginBottom: 28 }}>
        <span className="text-gradient-gold">ویرایش کتاب</span>
      </h1>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {genres.map(genre => (
                <button key={genre.id} onClick={() => toggleGenre(genre.id)} style={{
                  ...
                  border: `1px solid ${selectedGenres.includes(genre.id) ? 'var(--accent-gold)' : 'var(--border-default)'}`,
                  background: selectedGenres.includes(genre.id) ? 'var(--accent-gold-glow)' : 'var(--bg-elevated)',
                  color: selectedGenres.includes(genre.id) ? 'var(--accent-gold)' : 'var(--text-secondary)',
                }}>{genre.name}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
            {chapters.map((chapter, i) => (
              <button key={chapter.id ?? `new-${i}`} onClick={() => setActiveChapter(i)} style={{
          ...
          {active && (
            <motion.div key={active.id ?? activeChapter} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div>
                ...
                <input value={active.title} onChange={e => updateChapterDraft(activeChapter, 'title', e.target.value)} placeholder="عنوان فصل..." style={inputStyle} />
              </div>

              <div className="chapter-meta-row">
                <div style={{ flex: 1 }}>
                  <label style={labelStyle}>قیمت (سکه)</label>
                  <input type="number" value={active.price} onChange={e => updateChapterDraft(activeChapter, 'price', Number(e.target.value))} min={0} style={{ ...inputStyle, direction: 'ltr' }} disabled={active.isFree} />
                </div>
                <div style={{ display: 'flex', alignItems: 'flex-end', paddingBottom: 1 }}>
                  <button onClick={() => updateChapterDraft(activeChapter, 'isFree', !active.isFree)} style={{
                    padding: '9px 16px', borderRadius: 'var(--radius-md)', cursor: 'pointer', fontFamily: 'inherit', fontSize: '0.85rem',
                    border: `1px solid ${active.isFree ? 'var(--accent-emerald)' : 'var(--border-default)'}`,
                    background: active.isFree ? 'rgba(16,185,129,0.15)' : 'var(--bg-elevated)',
                    color: active.isFree ? 'var(--accent-emerald)' : 'var(--text-secondary)',
                  }}>
                    {active.isFree ? 'رایگان' : 'رایگان؟'}
                  </button>
                </div>
                <div>
                  <label style={labelStyle}>وضعیت</label>
                  <select value={active.status} onChange={e => updateChapterDraft(activeChapter, 'status', e.target.value)} style={{ ...inputStyle, cursor: 'pointer' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <textarea
                  ...
                  onChange={e => updateChapterDraft(activeChapter, 'content', e.target.value)}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24, gap: 10 }}>
        ...
        <Button isLoading={isSubmitting} onClick={handleSubmit} size="lg">ذخیره تغییرات</Button>
      </div>
    </div>
  );
}
